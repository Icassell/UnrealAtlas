...
void UAtlasToolsLibrary::ShowAtlasImage()
{
	...
	if (bSuccess)
	{
		...
		int RelativeIndex = StringArray.Find("Content");
		if (RelativeIndex != INDEX_NONE)
		{
			for (int i = RelativeIndex + 1; i < StringArray.Num(); i++)
			{
				RelativePath = RelativePath + StringArray[i] + "/";
			}
		}
		if (RelativePath.EndsWith("/"))
		{
			RelativePath = RelativePath.Left(RelativePath.Len() - 1);
		}
		//FString RelativePath = SelectFilePath.Right(SelectFilePath.Len() - FString("../../../Content").Len());
...
int32 UAtlasToolsLibrary::GenerateAtlasAllInFolders(TArray<FString> FolderPaths, bool bRecreateAtlas /* = true */, bool bClearSprite /* = true */)
{
	...
	{
		int32 res = GenerateAtlasAllInFolder(Folder, bRecreateAtlas, bClearSprite);
		if (res != 0)
		{
			return -1;
		}
...
int32 UAtlasToolsLibrary::GenerateAtlasAllInFolder(FString Path, bool bRecreateAtlas /* = true */, bool bClearSprite /* = true */)
{
	if (Path == "")
	{
		return -1;
	...
	if (bRecreateAtlas)
	{
		...
		bool bSaveSuccess = UEditorAssetLibrary::SaveAsset(FAssetData(PaperSpriteAtlasNew).ObjectPath.ToString());
		FString Ppath = FAssetData(PaperSpriteAtlasNew).ObjectPath.ToString();
		UE_LOG(LogTemp, Log, TEXT("%s"), *Ppath);
	}
	else
	{
	...
	if (bClearSprite)
	{
		...
		filterDeletaSprite.bRecursivePaths = true;
	...
	filterAtlas.bRecursivePaths = true;
	...
	ArrayAssetData.Empty();
	bool bGetSuccess = assetRegistryModule.Get().GetAssets(filterAtlas, ArrayAssetData);
...
void UAtlasToolsLibrary::SetImageToAtlasAllInFolder(bool bRecreateAtlas /*= true*/, bool bClearSprite /*= true*/)
{
	...
	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	const bool bSuccess = DesktopPlatform->OpenDirectoryDialog(nullptr, TEXT("AtlasDialog"), DefaultPath, SelectPath);
	if (bSuccess)
	{
		GenerateAtlasAllInFolder(SelectPath, bRecreateAtlas, bClearSprite);
	}
}
...
bool UAtlasToolsLibrary::AutoAdjustSpriteAtlasTextureSize(class UPaperSpriteAtlas* PaperSpriteAtlas)
{
	...
	if (SpriteMaxSizes.Num() == 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("SpriteMaxSizes is Empty!"));
		return false;
	}
